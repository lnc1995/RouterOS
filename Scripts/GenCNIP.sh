...
grep ':' cn.list | sed -e 's/^/add address=/g' -e 's/$/ list=CNIP_List_V6/g' > 5.rsc
...
sort -u 5.rsc | tr -d '\r' | grep -v '^[[:space:]]*$' | sed -e '1i/ipv6 firewall address-list \nremove [/ipv6 firewall address-list find list=CNIP_List_V6]' | awk 'NR>1{print buf} {buf=$0} END{printf "%s", buf}' > CNIP_List_V6.RSC
rm -rf *.list
rm -rf *.rsc
